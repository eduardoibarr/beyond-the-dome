...
from settings import (
    PLAYER_HEALTH, HEALTH_BAR_WIDTH, HEALTH_BAR_HEIGHT, HEALTH_BAR_X, HEALTH_BAR_Y,
    HEALTH_BAR_COLOR_MIN, HEALTH_BAR_COLOR_MAX, HEALTH_BAR_BACKGROUND_COLOR,
    HEALTH_BAR_BORDER_COLOR, RADIATION_MAX, RADIATION_BAR_X, RADIATION_BAR_Y,
    RADIATION_BAR_WIDTH, RADIATION_BAR_HEIGHT, RADIATION_BAR_COLOR_MIN,
    RADIATION_BAR_COLOR_MAX, RADIATION_BAR_BACKGROUND_COLOR, RADIATION_BAR_BORDER_COLOR,
    HUD_COLOR, PISTOL_MAGAZINE_SIZE, FONT_NAME, HUD_FONT_SIZE,
    GREEN, RED, YELLOW, WHITE # Import colors used in HUD
)
import math # For pulsing effect

def draw_hud(game):
    """Draws the Heads-Up Display (Health, Radiation, Ammo, Filter)."""
    screen = game.screen
    player = game.player
    font = game.font # Assuming game object has the font loaded
    shadow_offset = 1
    border_radius = 8
    border_thickness = 1
    ...
    if hasattr(player, 'health'):
        ...
        # Interpolate color
        # ... (color interpolation logic as before) ...
        ...
        # Draw background
        # ... (background drawing logic as before) ...
        bg_surface = pygame.Surface((bar_width, bar_height), pygame.SRCALPHA)
        pygame.draw.rect(bg_surface, (*HEALTH_BAR_BACKGROUND_COLOR, 180), bg_surface.get_rect(), border_radius=border_radius)
        ...
        # ... (border/highlight logic as before) ...
        border_rect = pygame.Rect(bar_x, bar_y, bar_width, bar_height)
        pygame.draw.rect(screen, (*HEALTH_BAR_BORDER_COLOR, 180), border_rect, border_thickness, border_radius=border_radius)
        highlight_height = max(2, int(bar_height * 0.2))
        highlight_rect = pygame.Rect(bar_x + border_thickness, bar_y + border_thickness, bar_width - 2*border_thickness, highlight_height)
        highlight_surface = pygame.Surface((highlight_rect.width, highlight_rect.height), pygame.SRCALPHA)
        pygame.draw.rect(highlight_surface, (255, 255, 255, 40), highlight_surface.get_rect(), border_radius=border_radius-1)
        screen.blit(highlight_surface, highlight_rect)

        # Draw text
        # ... (text drawing logic as before) ...
    ...
    if hasattr(player, 'radiation'):
        ...
        current_rad_color = (max(0, min(255, rad_color_r)), max(0, min(255, rad_color_g)), max(0, min(255, rad_color_b)))
        rad_color_end = (max(0, min(255, rad_color_r - 30)), max(0, min(255, rad_color_g - 30)), max(0, min(255, rad_color_b - 30)))

        # Draw background
        # ... (background drawing logic as before) ...
        rad_bg_surface = pygame.Surface((rad_bar_width, rad_bar_height), pygame.SRCALPHA)
        pygame.draw.rect(rad_bg_surface, (*RADIATION_BAR_BACKGROUND_COLOR, 180), rad_bg_surface.get_rect(), border_radius=border_radius)
        screen.blit(rad_bg_surface, (rad_bar_x, rad_bar_y))

        # Draw fill with gradient and pulse
        # ... (fill drawing logic as before) ...
        if rad_fill_width > 0:
            ...
            pygame.draw.rect(rad_mask_surf, (255, 255, 255), rad_mask_surf.get_rect(), border_radius=border_radius)
        ...
        pygame.draw.rect(screen, (*RADIATION_BAR_BORDER_COLOR, 180), rad_border_rect, border_thickness, border_radius=border_radius)
        rad_highlight_height = max(2, int(rad_bar_height * 0.2))
        rad_highlight_rect = pygame.Rect(rad_bar_x + border_thickness, rad_bar_y + border_thickness, rad_bar_width - 2*border_thickness, rad_highlight_height)
        rad_highlight_surface = pygame.Surface((rad_highlight_rect.width, rad_highlight_rect.height), pygame.SRCALPHA)
        pygame.draw.rect(rad_highlight_surface, (255, 255, 255, 40), rad_highlight_surface.get_rect(), border_radius=border_radius-1)
    ...
    # --- Filter Module Indicator ---
    if hasattr(player, 'has_filter_module'):
        # ... (filter indicator logic as before, using GREEN/RED) ...
    ...
    if hasattr(player, 'pistol') and hasattr(player.pistol, 'ammo_in_mag') and hasattr(player, 'reserve_ammo'):
        ...
        ammo_rect = ammo_surface.get_rect(bottomright=(game.screen.get_width() - 15, game.screen.get_height() - 15))

        # Draw background
        # ... (background drawing logic as before) ...
        padding = 8
        ammo_bg_rect = pygame.Rect(ammo_rect.left - padding, ammo_rect.top - padding, ammo_rect.width + padding * 2, ammo_rect.height + padding * 2)
        ammo_bg_surface = pygame.Surface(ammo_bg_rect.size, pygame.SRCALPHA)
        pygame.draw.rect(ammo_bg_surface, (0, 0, 0, 100), ammo_bg_surface.get_rect(), border_radius=5)
